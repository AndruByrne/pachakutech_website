      ...
      SliverPadding(
        ...
        sliver: FutureBuilder(
          ...
          builder: (context, asyncSnapshot) {
            var blogEntries = asyncSnapshot.hasData ? asyncSnapshot.data ?? [] : [];
            return SliverList(
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  if (index < blogEntries.length) {
                    ...
                    return ListTile(
                      title: Text(entry['content']?[0]?['title'] ?? 'No Title'), // just testing
                    );
                  } else
                    return Card(
                      child: Column(
                ...
                childCount: blogEntries.length + 1, // Your desired child count
              ),
            );
          }
        ),
